...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3">
            <a href="{{ route('events.show', $event) }}" class="inline-flex items-center px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-lg">
        <form method="POST" action="{{ route('events.update', $event) }}" enctype="multipart/form-data" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="w-20 h-20 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
                                @if($event->logo)
                                <img src="{{ Storage::url($event->logo) }}" alt="Current logo" class="w-20 h-20 object-cover rounded-lg" id="current-logo">
                                @else
                            ...
                            <div class="flex-1">
                                ...
                                @if($event->logo)
                                <p class="mt-1 text-xs text-blue-600">Current: {{ basename($event->logo) }}</p>
                                @endif
                            </div>
                        </div>
                        @error('logo')
            ...
            <div class="border-b border-gray-200 pb-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-green-50 p-4 rounded-lg">
                        <div class="text-2xl font-bold text-green-600">{{ $event->tickets->count() }}</div>
                        <div class="text-sm text-gray-600">Ticket Types</div>
                    </div>
                    <div class="bg-purple-50 p-4 rounded-lg">
                        <div class="text-2xl font-bold text-purple-600">{{ $event->registrationFields->count() }}</div>
                        <div class="text-sm text-gray-600">Registration Fields</div>
                    </div>
                    <div class="bg-orange-50 p-4 rounded-lg">
                        <div class="text-2xl font-bold text-orange-600">{{ ucfirst($event->status) }}</div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6">
                <a href="{{ route('events.show', $event) }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
                    Cancel
                </a>
